...
class LLMAnalyzer:
    ...
    """
    def __init__(self, api_keys: List[str] = None, base_url: str = "http://10.129.80.218:3000/v1", model: str = "deepseek-v3-250324"):
        """
        初始化LLM分析器
        ...
        if isinstance(api_keys, str):
            ...
            self.api_keys = ["sk-XnbHbzBOmPYGHgL_xCMCYcABqSNTuqAhKtmOXJEONih7BqRZSw-JuRM1RbI"]  # 默认密钥
    ...
    def _make_request(self, messages: List[Dict], temperature: float = 0.7, max_tokens: int = 1000) -> str:
        """
        向大语言模型发送请求
    ...
    def _make_parallel_request(self, messages: List[Dict], temperature: float = 0.7, max_tokens: int = 1000, client_index: int = 0) -> str:
        """
        并行请求的单个调用，使用指定的客户端
    ...
    def batch_analyze_stocks_parallel(self, analysis_tasks: List[Dict]) -> List[Dict]:
        ...
        self.logger.info(f"并行分析完成，成功: {sum(1 for r in results if r['success'])}/{len(results)}")
        return results
    def _analyze_sentiment_single(self, stock_symbol: str, recent_data: pd.DataFrame, 
                                 news_headlines: List[str] = None, client_index: int = 0) -> Dict[str, Any]:
        """
        单个股票情感分析（并行版本）- 增强版包含30天历史数据
        """
        if recent_data.empty:
            return {"sentiment_score": 0.5, "confidence": 0.5, "reasoning": "无数据", "risk_level": "中"}
        
        # 计算基本统计信息
        latest_price = recent_data['Close'].iloc[-1]
        price_change = (latest_price - recent_data['Close'].iloc[0]) / recent_data['Close'].iloc[0] * 100
        volatility = recent_data['Close'].pct_change().std() * 100
        volume_trend = recent_data['Volume'].iloc[-5:].mean() / recent_data['Volume'].iloc[:-5].mean() if len(recent_data) >= 10 else 1
        ...
        if len(recent_data) >= 30:
            for i in range(-30, 0, 5):  # 每5天采样一次
                ...
                if idx < len(recent_data):
                    price_history.append(f"{recent_data.iloc[idx]['Date'].strftime('%m-%d')}: ${recent_data.iloc[idx]['Close']:.2f}")
...
"""
        ...
        messages = [
            {"role": "system", "content": "你是一位经验丰富的金融分析师，擅长股票市场分析和情感分析。"},
            {"role": "user", "content": prompt}
        ]
        
        response = self._make_parallel_request(messages, temperature=0.3, client_index=client_index)
        try:
    ...
    def _generate_trading_signal_single(self, stock_symbol: str, technical_indicators: Dict[str, float],
                                       market_sentiment: Dict[str, Any], client_index: int = 0) -> Dict[str, Any]:
        """
        单个股票交易信号生成（并行版本）
        """
        prompt = f"""
...
"""
        messages = [
            {"role": "system", "content": "你是一位专业的量化交易分析师，擅长结合技术分析和基本面分析生成交易信号。"},
            {"role": "user", "content": prompt}
        ]
        
        response = self._make_parallel_request(messages, temperature=0.2, client_index=client_index)
        try:
...
"""
    ...
    def analyze_stock_relationships(self, stock_symbols: List[str], 
...
"""
        ...
        for pair in high_corr_pairs[:10]:  # 只显示前10个
            prompt += f"- {pair['stock1']} vs {pair['stock2']}: {pair['correlation']:.3f}\n"
...
"""
        messages = [
            {"role": "system", "content": "你是一位专业的投资组合经理，擅长行业分析和风险管理。"},
            {"role": "user", "content": prompt}
        ]
    ...
    def generate_trading_signals(self, stock_symbol: str, technical_indicators: Dict[str, float],
...
"""
    ...
    def analyze_risk_factors(self, portfolio_data: Dict[str, pd.DataFrame], 
...
"""
        messages = [
            {"role": "system", "content": "你是一位专业的投资风险管理专家，擅长投资组合风险评估。"},
            {"role": "user", "content": prompt}
        ]
    ...
    def _get_company_context(self, stock_symbol: str) -> str:
        ...
        company_info = {
            ...
            'MSFT': {
                ...
                'market_cap': '大盘股',
                'key_factors': ['云计算领导地位', 'Office套件', 'Azure平台', '企业客户基础']
            },
            'GOOGL': {
                'name': 'Alphabet Inc.',
                'industry': '互联网技术',
