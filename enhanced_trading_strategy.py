...
class EnhancedTradingStrategy:
    ...
    增强版交易策略，集成大语言模型分析
    """
    def __init__(self, stock_pool, llm_api_keys, llm_base_url="http://10.129.80.218:3000/v1"):
        """
        初始化增强版交易策略
    ...
    def _create_features(self, df):
        ...
        df["sma_10"] = ta.trend.sma_indicator(df["Close"], window=10) if len(df) >= 10 else np.nan
        df["sma_20"] = ta.trend.sma_indicator(df["Close"], window=20) if len(df) >= 20 else np.nan
        ...
        if stock_symbol in self.sentiment_history:
            # 使用最近的情感分析结果
            recent_sentiment = self.sentiment_history[stock_symbol][-1] if self.sentiment_history[stock_symbol] else 0.5
    ...
    def build_stock_relationship_network(self, all_data):
        ...
        self.logger.info(f"股票关系网络构建完成: {self.stock_network.number_of_nodes()}个节点, {self.stock_network.number_of_edges()}条边")
    ...
    def analyze_market_sentiment_for_all_stocks(self, all_data, current_date):
        ...
        for stock_symbol in self.stock_pool:
            ...
            recent_data = current_data.tail(20)  # 最近20天的数据
            ...
            try:
                ...
                if len(self.sentiment_history[stock_symbol]) > 100:
                    self.sentiment_history[stock_symbol] = self.sentiment_history[stock_symbol][-50:]
    ...
    def generate_llm_trading_signals(self, all_data, current_date):
        ...
        for stock_symbol in self.stock_pool:
            ...
            technical_indicators = {
                ...
                'sma_ratio': latest_data.get('Close', 0) / latest_data.get('sma_20', 1) if latest_data.get('sma_20', 0) != 0 else 1,
                'atr': latest_data.get('atr', 0),
                'volume_ratio': latest_data.get('Volume', 0) / enhanced_data['Volume'].rolling(20).mean().iloc[-1] if enhanced_data['Volume'].rolling(20).mean().iloc[-1] != 0 else 1
            }
            
            # 获取情感分析结果
            market_sentiment = {}
            if stock_symbol in self.sentiment_history and self.sentiment_history[stock_symbol]:
        ...
        for decision in traditional_decisions:
            ...
            if llm_signal and 'signal' in llm_signal:
                ...
                if llm_action == '卖出' and original_action == 'buy':
                    ...
                    if llm_confidence > 0.7:
                        enhanced_decision['action']['shares'] = int(decision['action']['shares'] * 0.3)
                        enhanced_decision['reason'] += f" [LLM警告: {llm_signal.get('reasoning', '')}]"
                elif llm_action == '买入' and original_action == 'buy':
                    # 两者都建议买入，可能增加买入量
                    if llm_confidence > 0.7:
                        enhanced_decision['action']['shares'] = int(decision['action']['shares'] * 1.2)
                        enhanced_decision['reason'] += f" [LLM确认: {llm_signal.get('reasoning', '')}]"
                elif llm_action == '买入' and original_action == 'sell':
                    # LLM建议买入但技术分析建议卖出，减少卖出量
                    if llm_confidence > 0.7:
                        enhanced_decision['action']['shares'] = int(decision['action']['shares'] * 0.5)
                        enhanced_decision['reason'] += f" [LLM反对: {llm_signal.get('reasoning', '')}]"
            ...
            if sentiment_data:
                ...
                if risk_level == '高' and enhanced_decision['action']['type'] == 'buy':
                    enhanced_decision['action']['shares'] = int(enhanced_decision['action']['shares'] * 0.7)
                    enhanced_decision['reason'] += f" [高风险调整]"
                elif risk_level == '低' and enhanced_decision['action']['type'] == 'buy':
                    enhanced_decision['action']['shares'] = int(enhanced_decision['action']['shares'] * 1.1)
    ...
    def make_decision(self, current_data, date):
        ...
        for stock_symbol, data_list in current_data.items():
            ...
            # 转换数据格式
            df = pd.DataFrame(data_list, columns=['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'Name'])
            ...
            if signal > 2:  # 强买信号
                decisions.append({
                    'action': {
                        'type': 'buy',
                        'symbol': stock_symbol,
                        'shares': min(2000, int(signal * 400)),  # 根据信号强度调整数量
                        'price': price
                    },
    ...
    def _calculate_signal_strength(self, df, latest_data):
        ...
        if pd.isna(macd):
            macd = 0
        macd_prev = df['macd'].iloc[-2] if len(df) > 1 and not pd.isna(df['macd'].iloc[-2]) else 0
    ...
    def get_analysis_summary(self):
        ...
        """
        summary = {
            'sentiment_analysis_count': sum(len(history) for history in self.sentiment_history.values()),
    ...
    def analyze_market_sentiment_for_all_stocks_parallel(self, all_data, current_date):
        ...
        for stock_symbol in self.stock_pool:
            ...
            # 获取最近的数据用于分析
            recent_data = current_data.tail(20)
        ...
        for result in results:
            if result['success'] and result['result']:
                ...
                self.logger.info(f"{stock_symbol} 情感分析完成: 评分={sentiment_data.get('sentiment_score', 0.5):.3f}")
    ...
    def generate_llm_trading_signals_parallel(self, all_data, current_date):
        ...
        for stock_symbol in self.stock_pool:
            ...
            if stock_symbol in self.sentiment_history and self.sentiment_history[stock_symbol]:
                ...
                market_sentiment = {'sentiment_score': 0.5, 'confidence': 0.5, 'risk_level': '中'}
        ...
        for result in results:
            if result['success'] and result['result']:
                ...
                self.logger.info(f"{stock_symbol} 交易信号: {signal_data.get('signal', '持有')} (置信度: {signal_data.get('confidence', 0.5):.3f})")
