...
我并没有单一地采用LLM的结果作为最终决策，而是结合了基本的技术分析决策策略（利用RSI，MACD，移动平均值，布林带信号产生交易信号），**并根据二者置信度结合两种策略**，结合方式如下：若二者交易信号一致（即都买入或者都卖出）：则增强信号
...
![stock_relationship_network](报告/stock_relationship_network.png)
