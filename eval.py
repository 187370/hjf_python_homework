...
def evaluate_strategy(
    strategy_class, all_stock_data, initial_cash=1000000, num_trading_days=60
):
    """
    评测函数实现
    ...
    portfolio = {
        ...
        "holdings": defaultdict(
            int
        ),  # {stock: shares}，正值表示多头持仓，负值表示空头持仓
        "transaction_log": [],
    }
    ...
    for i in tqdm(range(num_trading_days - 1)):  # 最后一天不需要生成策略
        ...
        for strategy_item in strategy_list:
            ...
            for stock, order in strategy_item.items():
                ...
                shares1 = min(
                    order.get("shares1", 0), max_shares_per_trade
                )  # 限制单次交易股数
                ...
                shares2 = min(
                    order.get("shares2", 0), max_shares_per_trade
                )  # 限制单次交易股数
